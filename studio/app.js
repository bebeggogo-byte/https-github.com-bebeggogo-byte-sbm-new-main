...
(function () {
  ...
  function buildPptx(slides, renderFn, size) {
    size = size || { w: 12192000, h: 6858000 };
    const NS = PPTX_NS;
...
<p:presentation ${NS}><p:sldMasterIdLst><p:sldMasterId id="2147483648" r:id="rId1"/></p:sldMasterIdLst><p:sldIdLst>${slides.map((_, i) => `<p:sldId id="${256 + i}" r:id="rId${i + 2}"/>`).join('')}</p:sldIdLst><p:sldSz cx="${size.w}" cy="${size.h}"/><p:notesSz cx="6858000" cy="9144000"/></p:presentation>`
    });
    files.push({
...
${blocks}`;
  ...
  /* ---------- 워크지(A4 활동지) — 매 활동: 가이드 + 명료한 예시 + 응답 공간 ---------- */
  const A4 = { w: 7560000, h: 10692000, mx: 720000 }; // 210×297mm, 여백 20mm

  function normalizeWorksheet(parsed) {
    if (!parsed || typeof parsed !== 'object' || !Array.isArray(parsed.items)) return null;
    const S = (x) => String(x == null ? '' : x).trim();
    const items = parsed.items.map(it => {
      if (!it) return null;
      let type = S(it.type).toLowerCase();
      if (!['activity', 'question', 'fill'].includes(type)) type = 'activity';
      return {
        type, title: S(it.title),
        guide: Array.isArray(it.guide) ? it.guide.map(S).filter(Boolean).slice(0, 4) : [],
        example: S(it.example), prompt: S(it.prompt)
      };
...
{"theme":"cobalt","orientation":"portrait","orientationReason":"순차 작성 중심이라 세로","kicker":"시리즈·강의명","title":"워크지 제목","objective":"학습목표 한 줄","items":[{"type":"activity","title":"활동명","guide":["단계"],"example":"예) ...","prompt":"응답 지시"},{"type":"question","prompt":"깊이 질문"},{"type":"fill","prompt":"문장 속 ____ 빈칸"}]}`;
  ...
  function buildWorksheetPromptFromPlan(P) {
    const body = P.deep ? buildPlanMd(P) : `제목: ${P.title}\n${P.intent ? '설계 의도: ' + P.intent + '\n' : ''}슬라이드 개요:\n${P.slidesText || '(없음)'}`;
    return `당신은 참여형 교육 워크지(활동지) 설계자입니다. 아래 강의 설계에 맞는 A4 워크지를 만드세요.\n\n${worksheetRules()}\n\n[강의 설계]\n${body}`;
  }
  function buildWorksheetPromptFromAtoms(theme, aud, picked) {
    const blocks = picked.map((a, i) => `#${i + 1} [${a.type}] ${a.title}\n${a.content || a.summary}`).join('\n\n');
    return `당신은 참여형 교육 워크지(활동지) 설계자입니다. 아래 재료로 「${theme || '새 강의'}」 A4 워크지를 만드세요.${aud ? '\n[대상] ' + aud : ''}\n\n${worksheetRules()}\n\n[재료]\n${blocks}`;
  }

  function openWorksheetPaste(baseName) {
    openModal('워크지 결과(JSON) 붙여넣기', `
      <div class="stack">
        <p class="muted small">클로드가 준 JSON을 붙여넣으면 A4 워크지(.pptx)로 저장됩니다 — 파워포인트에서 편집·인쇄하세요.</p>
        <textarea id="wsIn" class="mono" style="min-height:200px" placeholder='{"title":"...","items":[...]}'></textarea>
        <div class="row"><button class="btn primary" id="wsApply">A4 .pptx 만들기</button><button class="btn ghost" id="wsCancel">취소</button></div>
      </div>`, () => {
  ...
  async function autoWorksheet(promptText, baseName, btn) {
    if (!state.settings.anthropicKey) { toast('설정에서 Anthropic API 키를 먼저 입력하세요'); return; }
    btn.disabled = true; const old = btn.textContent; btn.textContent = '워크지 제작 중…';
    try {
  ...
  function openPlan(id) {
    ...
    openModal(P.title, `
      <div class="stack">
        ...
        <div class="card" style="box-shadow:none">
          ...
          <div class="row" style="margin-top:8px">
            <button class="btn sm" id="plWsPrompt">📝 ① 워크지(A4) 프롬프트</button>
            <button class="btn sm" id="plWsPaste">📝 ② 결과(JSON) → A4</button>
            ${state.settings.anthropicKey ? `<button class="btn sm green" id="plWsAuto">⚡ 자동 워크지</button>` : ''}
          </div>
          <div class="row" style="margin-top:8px">
      ...
      $('#plWsPrompt').addEventListener('click', async () => { await save(true); copy(buildWorksheetPromptFromPlan(P)); });
      $('#plWsPaste').addEventListener('click', async () => { await save(true); openWorksheetPaste(P.title); });
      const wsa = $('#plWsAuto'); if (wsa) wsa.addEventListener('click', async () => { await save(true); autoWorksheet(buildWorksheetPromptFromPlan(P), P.title, wsa); });
    });
  }
...
${(L.transcript || '').trim()}`;
  ...
  RENDER.compose = function () {
        ...
        <div class="tray">
          <div class="card">
            ...
            <div class="stack" style="margin-top:10px">
              ...
              <button class="btn block" id="cWsPrompt" ${picked.length ? '' : 'disabled'}>📝 워크지(A4) 프롬프트</button>
              <button class="btn block" id="cWsPaste" ${picked.length ? '' : 'disabled'}>📝 결과(JSON) → A4</button>
    ...
    bind('cWsPrompt', 'click', () => copy(buildWorksheetPromptFromAtoms(ui.composeTheme.trim(), ui.composeAudience.trim(), picked)));
    bind('cWsPaste', 'click', () => openWorksheetPaste(ui.composeTheme.trim() || '새강의'));
