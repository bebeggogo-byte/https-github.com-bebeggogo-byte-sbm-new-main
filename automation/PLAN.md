...
## 알려진 한계 (실제 혼합 데이터 감사 결과)
혼합 인입(카톡·문자·에이닷·이메일)으로 끝까지 돌려 확인한 잔여 한계:

- **모호한 날짜**: `다음 달 셋째 주`, `세부 일정 조율 후` 등은 해석 불가 → 날짜 미정으로
  남고 needs_review 에 표시. (`내일`·`다음주 화요일`·`2026년 6월 20일`은 해석됨)
- **날짜 미확정 건**: 캘린더 단계는 **자동 건너뜀(⊘)** 하고 커리큘럼·자료는 계속 생성.
  일정이 확정되면 다시 `run` 으로 .ics 생성.
- **테마 분류는 '내용 키워드' 기반**: 발신자가 알던 고객이라도 메시지에 그 고객 키워드가
  없으면 default 로 분류됨. → 고객을 확실히 묶으려면 `--theme <고객>` 지정하거나
  `themes/<고객>.json` 의 `match` 에 발신자/별칭을 추가.
- **장소 '추후 통보'**: 안내 문구를 장소로 오인하지 않고 미정으로 표시.
